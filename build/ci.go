...
import (
	...
	"github.com/LCC/go-LCC/internal/build"
)
...
var (
	...
	debExecutables = []debExecutable{
		{
			Name:        "abigen",
			Description: "Source code generator to convert LCC contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			Name:        "bootnode",
			Description: "LCC bootnode.",
		},
		{
			Name:        "evm",
			Description: "Developer utility version of the EVM (LCC Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			Name:        "geth",
			Description: "LCC CLI client.",
		},
		{
			Name:        "puppeth",
			Description: "LCC private network manager.",
		},
		{
			Name:        "rlpdump",
		...
		{
			Name:        "swarm",
			Description: "LCC Swarm daemon and tools",
		},
		{
			Name:        "wnode",
			Description: "LCC Whisper diagnostic tool",
		},
	}
...
func doInstall(cmdline []string) {
	...
	if runtime.Version() < "go1.7" && !strings.Contains(runtime.Version(), "devel") {
		...
		log.Println("go-LCC requires at least Go version 1.7 and cannot")
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:LCC/LCC")`)
...
type debMetadata struct {
	...
	// go-LCC version being built. Note that this
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.LCC", "-v", "github.com/LCC/go-LCC/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "--target", "ios", "--tags", "ios", "-v", "github.com/LCC/go-LCC/mobile")
