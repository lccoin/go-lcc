...
// This file is part of go-LCC.
//
// go-LCC is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-LCC is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-LCC. If not, see <http://www.gnu.org/licenses/>.
...
import (
	...
	"github.com/LCC/go-LCC/p2p/discover"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the LCC protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for LCC and the faucet")
)
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // LCC address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an LCC light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // LCC protocol stack
	client *ethclient.Client   // Client connection to the LCC chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the LCC light client protocol
...
// close terminates the LCC connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for LCC addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No LCC address found to fund"})
			continue
		}
