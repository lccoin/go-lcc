...
// Package utils contains internal helper functions for go-LCC commands.
package utils
...
import (
	...
	"github.com/LCC/go-LCC/p2p/netutil"
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(DevModeFlag.Name):
		cfg.DataDir = filepath.Join(os.TempDir(), "LCC_dev_mode")
	case ctx.GlobalBool(TestnetFlag.Name):
...
// RegisterEthService adds an LCC client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the LCC service: %v", err)
	}
}
...
// RegisterEthStatsService configures the LCC Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the LCC Stats service: %v", err)
	}
}
