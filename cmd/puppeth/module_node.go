...
// nodeDockerfile is the Dockerfile required to run an LCC node.
var nodeDockerfile = `
FROM LCC/client-go:latest
...
RUN \
  echo 'geth init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.LCC/keystore/ && cp /signer.json /root/.LCC/keystore/' >> geth.sh && \{{end}}
...
// an LCC node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new LCC node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.LCC"],
