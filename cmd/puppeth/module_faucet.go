...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/LCC                                                            && \
	(cd $GOPATH/src/github.com/LCC && git clone --depth=1 https://github.com/LCC/go-LCC) && \
  go build -v github.com/LCC/go-LCC/cmd/faucet                                              && \
