...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.LCC", new NodeConfig());
			node.start();
			...
			LCCClient ec = node.getLCCClient();
	...
	// signatures too. Regression test for go-LCC issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/LCC/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.LCC", "github.com/LCC/go-LCC/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "LCC", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
