...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the LCC
...
type wallet struct {
	...
	deriveChain    LCC.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived LCC account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain LCC.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
