...
import (
	...
	"github.com/LCC/go-LCC/accounts/usbwallet/internal/trezor"
...
// or whether the LCC app was not started on it.
...
// LCC address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.LCCAddress)
	if _, err := w.trezorExchange(&trezor.LCCGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.LCCTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.LCCTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
