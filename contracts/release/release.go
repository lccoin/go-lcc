...
type Config struct {
	Oracle common.Address // LCC address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the LCC service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var LCC *eth.LCC
	if err := ctx.Service(&LCC); err == nil {
		apiBackend = LCC.ApiBackend
	} else {
		var LCC *les.LightLCC
		if err := ctx.Service(&LCC); err == nil {
			apiBackend = LCC.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/LCC/go-LCC/releases for new releases")
