...
import (
	...
	"github.com/LCC/go-LCC/accounts/keystore"
	"github.com/LCC/go-LCC/common"
	"github.com/LCC/go-LCC/common/hexutil"
	"github.com/LCC/go-LCC/common/math"
...
// PublicLCCAPI provides an API to access LCC related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicLCCAPI struct {
	b Backend
}

// NewPublicLCCAPI creates a new LCC protocol API.
func NewPublicLCCAPI(b Backend) *PublicLCCAPI {
	return &PublicLCCAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicLCCAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current LCC protocol version this node supports
func (s *PublicLCCAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicLCCAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19LCC Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19LCC Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an LCC ECDSA signature for:
// keccack256("\x19LCC Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/LCC/go-LCC/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19LCC Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/LCC/go-LCC/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid LCC signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the LCC blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new LCC blockchain API.
...
//
// https://github.com/LCC/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of LCC APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of LCC APIs exposed over the private
// debugging endpoint.
...
// Version returns the current LCC protocol version.
